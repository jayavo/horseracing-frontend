...
function getColor(v) { return v >= 80 ? C.accent : v >= 60 ? C.warn : C.danger; }
function getBg(v) { return v >= 80 ? '#0e2b20' : v >= 60 ? '#2b1e0a' : '#2b0e0e'; }
...
function HorseCard({ runner, rank, raceId, raceDetail }) {
  ...
  const lastFive = formHistory.slice(0, 5).map(f => f.position);

  async function doDeepDive() {
    setLoadingDD(true); setTab('deepdive');
    try {
      const res = await horseDeepDive(raceId, runner.horse_name);
      setDeepDive(res.data.analysis);
    } catch (err) {
      setDeepDive('Deep dive analysis failed. Please try again.');
  ...
  const pWins = formHistory.filter(f => f.jockey === runner.jockey && f.position === 1).length;
  ...
  const classHistory = formHistory.reduce((acc, f) => {
    ...
    acc[cls].runs++;
    if (f.position === 1) acc[cls].wins++;
    return acc;
  }, {});

  const usualClass = Object.entries(classHistory).sort((a, b) => b[1].runs - a[1].runs)[0]?.[0] || '—';
  const todayClass = raceDetail?.race_class || '—';
  const classMap = { 'Grade 1': 1, 'Grade 2': 2, 'Grade 3': 3, 'Listed': 4, 'Class 1': 5, 'Class 2': 6, 'Class 3': 7, 'Class 4': 8, 'Class 5': 9, 'Class 6': 10 };
  ...
  const oddsFrac = runner.odds?.fractional || '—';
  const oddsDecimal = runner.odds?.decimal ? `(${runner.odds.decimal.toFixed(2)})` : '';
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, cursor: 'pointer' }} onClick={() => setOpen(!open)}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            {trainerSR >= 30 && <Badge text="🔥 Hot yard" type="green" />}
            {jockeySR >= 30 && <Badge text="🎯 Top jockey" type="purple" />}
            {(s.handicap_position || 0) >= 82 && <Badge text="Fav weight" type="blue" />}
            {(s.ground_suit || 0) >= 80 && <Badge text="Ground suits" type="green" />}
            {(s.ground_suit || 0) < 55 && <Badge text="Ground concern" type="red" />}
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 18, fontWeight: 600, color: C.text }}>{oddsFrac}</div>
            <div style={{ fontSize: 11, color: C.muted }}>{oddsDecimal}</div>
          </div>
        ...
        <div style={styles.detail}>
          ...
          {tab === 'class' && (
            <div>
              ...
              <div style={{ marginTop: 12, fontSize: 12, color: C.muted }}>Class history (wins/runs)</div>
              <div style={{ marginTop: 8 }}>
                {Object.entries(classHistory).sort((a,b)=>b[1].runs-a[1].runs).map(([cls, d]) => (
                  <div key={cls} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    <div style={{ fontSize: 11, color: cls === todayClass ? C.text : C.muted, minWidth: 100, fontWeight: cls === todayClass ? 600 : 400 }}>{cls}{cls === todayClass ? ' ★' : ''}</div>
          ...
          {tab === 'form' && (
            <div>
              {formHistory.length === 0 ? <div style={{ color: C.muted, fontSize: 13 }}>No form data available yet.</div> : (
                <table style={styles.formTable}>
                  <thead>
                    <tr style={{ fontSize: 11, color: C.muted }}>
                      {['Date','Course','Dist','Going','Class','Pos','OR','Jockey'].map(h => <th key={h} style={{ textAlign:'left', padding:'4px 8px', fontWeight:500 }}>{h}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {formHistory.map((f, i) => (
                      <tr key={i} style={{ fontSize: 12, borderTop: `1px solid ${C.border}` }}>
                        ...
                        <td style={{ ...styles.td, fontWeight: 600, color: f.position === 1 ? C.accent : f.position <= 3 ? C.warn : C.text }}>{f.position || '—'}/{f.runners || '—'}</td>
                ...
                <div key={sub} style={styles.tjCard}>
                  ...
                  {(stats || []).map(s => (
                    <div key={s.period} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                      ...
                      <span style={{ fontWeight: 600, color: getColor(parseFloat(s.strike_rate) * 2.5) }}>{parseFloat(s.strike_rate).toFixed(1)}%</span>
                    </div>
                  ))}
          ...
          {tab === 'deepdive' && (
            <div>
              {loadingDD ? (
                <div style={{ color: C.muted, fontSize: 13, padding: '20px 0' }}>🤖 Claude is analysing {runner.horse_name}...</div>
              ) : deepDive ? (
                <div style={styles.aiBox}>{deepDive.split('\n').map((line, i) => <p key={i} style={{ margin: '4px 0', lineHeight: 1.7 }}>{line}</p>)}</div>
              ) : (
                <button onClick={doDeepDive} style={styles.aiBtn}>Get AI deep dive on {runner.horse_name}</button>
              )}
            </div>
          )}
        </div>
...
export default function RaceDetail() {
  ...
  async function runAnalysis() {
    setAnalysing(true);
    try {
      const res = await triggerAnalysis(raceId);
  ...
  // Merge scored ordering with runner data
  ...
  const unscored = (race?.runners || []).filter(r => !scored.find(s => s.horse_name === r.horse_name));
      ...
      <div style={styles.body}>
        ...
        {!aiAnalysis && (
          <div style={styles.noAnalysis}>
            ...
            <div style={{ color: C.muted, marginTop: 8 }}>No AI analysis yet. Click "Run AI Analysis" to get Claude's expert verdict on this race.</div>
          </div>
        )}
        ...
        <div style={{ marginTop: 24 }}>
          ...
          {orderedRunners.map((runner, i) => (
            <HorseCard key={runner.horse_name} runner={runner} rank={i + 1} raceId={raceId} raceDetail={race} />
          ))}
          {unscored.map((runner, i) => (
            <HorseCard key={runner.horse_name} runner={runner} rank={orderedRunners.length + i + 1} raceId={raceId} raceDetail={race} />
          ))}
        </div>
      </div>
    </div>
